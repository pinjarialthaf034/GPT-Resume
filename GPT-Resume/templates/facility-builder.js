...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Initialize/Load drafts or clean state
...
async function saveResumeToSupabase() {
    ...
    try {
        const payload = {
            ...
            email: currentUser.email || resumeState.email || "",
        ...
        if (error) {
            ...
            if (currentUser.email) {
                ...
                if (fallbackError) {
                    ...
                    console.log("Supabase synced (legacy fallback): Facility resume draft saved.");
                }
            } else {
                console.warn("Supabase autosave failed:", error);
            }
        } else {
            console.log("Supabase synced securely with canonical user_id: Resume draft saved.");
        }
    } catch (err) {
    ...
    try {
        ...
        if (data && data.resume_data) {
            ...
            if (resumeState.fontFace) {
                const fontSelector = document.getElementById("fontSelector");
                if (fontSelector) {
                    fontSelector.value = resumeState.fontFace;
                }
            }
