document.addEventListener("DOMContentLoaded", () => {
    ...
    async function checkSupabaseSession() {
        if (!isSupabaseReady) return;
        try {
            const user = window.ResumeAuth ? await window.ResumeAuth.getCurrentUser() : null;
            if (user) {
                isLoggedIn = true;
                updateLoginButton();
            } else {
        ...
        supabase.auth.onAuthStateChange((event, session) => {
            if (session && session.user) {
                if (window.ResumeAuth) {
                    window.ResumeAuth.syncEphemeralCache(session.user);
                }
                ...
                if (window.ResumeAuth) {
                    window.ResumeAuth.clearEphemeralCache();
                }
                isLoggedIn = false;
                if (!isSigningOut) {
                    updateLoginButton();
    ...
    // Handle Form Submit (Supabase Auth Operations)
    if (authForm) {
        authForm.addEventListener("submit", async (e) => {
            ...
            try {
                if (isSignUpMode) {
                    // Register user via Supabase Auth as the single authority
                    ...
                    // Obtain active session if returned immediately
    ...
    if (goToBuilder) {
        goToBuilder.addEventListener("click", async () => {
            const user = window.ResumeAuth ? await window.ResumeAuth.getCurrentUser() : null;
            if (!user) {
                if (modalContentNotice) modalContentNotice.style.display = "none";
                if (modalContentForm) modalContentForm.style.display = "block";
                setAuthMode(false);
                pendingRedirect = "/resume/builder.html";
                openModal();
            } else {
                window.location.href = "/resume/builder.html";
            }
    ...
    if (goToGuidance) {
        goToGuidance.addEventListener("click", async () => {
            const user = window.ResumeAuth ? await window.ResumeAuth.getCurrentUser() : null;
            if (!user) {
                if (modalContentNotice) modalContentNotice.style.display = "none";
                if (modalContentForm) modalContentForm.style.display = "block";
                setAuthMode(false);
                pendingRedirect = "/career/dashboard.html";
                openModal();
            } else {
